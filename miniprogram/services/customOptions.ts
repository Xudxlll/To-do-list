...
function normalizeLegacyCustomOptionRecord(raw: unknown): LegacyCustomOptionRecord | null {
  const normalized = normalizeLegacyRecord(raw);
  if (!normalized) return null;
  return normalized;
}

export async function listCustomOptions(): Promise<LegacyCustomOptionRecord[]> {
  try {
    return (await listOptionCatalogRecords())
      .filter((record): record is LegacyCustomOptionRecord => !('recordType' in record))
      .map(cloneLegacyRecord);
  } catch (error) {
    console.warn('加载共享新标签失败', error);
    return [];
  }
}

export async function upsertCustomOptions(records: Array<{ categoryId: string; name: string }>): Promise<LegacyCustomOptionRecord[]> {
  const db = getCloudDb();
  const collection = db.collection(CLOUD_COLLECTIONS.customOptions);
  const saved: LegacyCustomOptionRecord[] = [];
  ...
  for (const input of records) {
    ...
    const legacyMatch = (Array.isArray(existed.data) ? existed.data : [])
      .map(normalizeLegacyCustomOptionRecord)
      .find((record): record is LegacyCustomOptionRecord => Boolean(record));
    if (legacyMatch) {
      saved.push(legacyMatch);
      continue;
    }

    const record: LegacyCustomOptionRecord = {
      categoryId,
      name,
      normalizedName,
      createdAt: Date.now(),
    ...
    try {
      ...
      const latest = await collection
        ...
        .limit(20)
        .get();
      const latestLegacy = (Array.isArray(latest.data) ? latest.data : [])
        .map(normalizeLegacyCustomOptionRecord)
        .find((item): item is LegacyCustomOptionRecord => Boolean(item));
      if (latestLegacy) {
        saved.push(latestLegacy);
        continue;
      }
      throw error;
    }
...
export async function deleteCustomOption(categoryId: string, name: string): Promise<void> {
  ...
  const legacyRecords = (Array.isArray(res.data) ? res.data : [])
    .map(normalizeLegacyCustomOptionRecord)
    .filter((record): record is LegacyCustomOptionRecord => Boolean(record));
